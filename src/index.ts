import { Dependencies, KeyValue, Type, json } from "./util";
...
export class Validator<T> {
  ...
  $id(id: string) {
    if (id[0] !== "/" && id[0] !== "#" && !id.startsWith("http")) {
      id = id.trim().replace(/\s*[A-Z]+/g, x => "-" + x.trim());
      if (id[0] === "-") {
        id = id.substr(1);
      }
    }
...
export class NumberValidator<T extends number = number> extends Validator<T> {
  type = Type.number;
...
export class ObjectValidator<T extends KeyValue = {}> extends Validator<T> {
  ...
  dependencies(dependencies: Dependencies<(keyof T) & string>): this {
...
export const v = {
  ...
  number: <T extends number = number>() => new NumberValidator<T>(),
  integer: <T extends number>() => new NumberValidator<T>().integer(),
  string: <T extends string>() => new StringValidator<T>(),
  object,
  array,
  tuple,
  map: <V = any>(v?: Validator<V>) => new ObjectValidator().additionalProperties(v || true),
  equal: <T>(item: T) => new Validator<T>({ "const": item }),
  enum: <T>(items: readonly T[]) => new Validator<T>({ "enum": items }),
  any: () => new Validator<any>(),
  anyOf,
  not: <T>(v: Validator<T>) => new Validator<Exclude<any, T>>({ not: v.toJSON() }),
  ref: (id: string) => new Validator({ $ref: id })
};
