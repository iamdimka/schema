...
function check(validator: Validator<any>, test: { schema?: any; ok?: any[], equals?: Array<[any, any]>, err?: any[] }) {
  if (test.schema) {
    try {
      assert.deepStrictEqual(validator.toJSON(), test.schema, "Schema is not equal")
    } catch (e) {
  ...
  const validator2 = validator.isOptional ? validator : new Validator<any>(validator.toJSON())

  if (Array.isArray(test.ok)) {
    for (const value of test.ok) {
      try {
        assert.deepStrictEqual(validator.validate(value), value)
      } catch (e) {
    ...
    for (const value of test.ok) {
      try {
        assert.deepStrictEqual(validator2.validate(value), value)
      } catch (e) {
        console.log(validator2.compile().toString())
        console.log(`${JSON.stringify(validator2)}: ${JSON.stringify(value)}`)
        assert.fail(e)
      }
    }
  }
  ...
  if (Array.isArray(test.equals)) {
    for (const [a, b] of test.equals) {
      try {
        assert.deepStrictEqual(validator.validate(a), b)
      } catch (e) {
        console.log(validator.compile().toString())
    ...
    for (const [a, b] of test.equals) {
      try {
        assert.deepStrictEqual(validator2.validate(a), b)
      } catch (e) {
        console.log(validator2.compile().toString())
        console.log(`${JSON.stringify(validator2)}: ${JSON.stringify(a)} is not equal to ${JSON.stringify(b)}`)
        assert.fail(e)
      }
    }
  }
  ...
  if (Array.isArray(test.err)) {
    ...
    for (const value of test.err) {
      ...
      console.log(validator2.schema)
      console.log(validator2.compile().toString())
      assert.fail(`Expected error on ${JSON.stringify(value)} : ${JSON.stringify(validator2)}`)
    }
  }
}
